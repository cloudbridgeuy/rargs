  ...
  while [[ $# -gt 0 ]]; do
    ...
    case "$key" in
      ...
      -m | --model)
        ...
        ;;
      --novelai-api-key)
        args['novelai-api-key']="$2"
        shift 2
        ;;
      --novelai-endpoint)
        args['novelai-endpoint']="$2"
        shift 2
        ;;
      -t | --temperature)
        args['temperature']="$2"
...
generate-stream() {
  ...
  if [[ -n "${NOVELAI_API_KEY:-}" ]]; then
    args['novelai-api-key']="${NOVELAI_API_KEY:-}"
  fi
  if [[ -n "${NOVELAI_ENDPOINT:-}" ]]; then
    args['novelai-endpoint']="${NOVELAI_ENDPOINT:-}"
  fi
  ...
  if [[ -z "${args['max-length']}" ]]; then
    args['max-length']="40"
  fi
  if [[ -z "${args['min-length']}" ]]; then
    args['min-length']="1"
  fi
  if [[ -z "${args['model']}" ]]; then
    args['model']="kayra-v1"
  fi
  if [[ -z "${args['novelai-endpoint']}" ]]; then
    args['novelai-endpoint']="https://api.novelai.net/ai/generate-stream"
  fi
  if [[ -z "${args['temperature']}" ]]; then
    args['temperature']="3"
  fi
  if [[ -z "${args['use-string']}" ]]; then
    args['use-string']="true"
  fi
  if [[ -z "${args['input']}" ]]; then
    args['input']="-"
  fi
  
  if [[ -n "${args['model']}" ]]; then
    if [[ ! "(kayra-v1 clio-v1 purple green red blue sigurd-2.9b-v1 cassandra infillmodel hypebot krake-v2 genji-jp-6b-v2 genji-python-6b euterpe-v2 6B-v4 2.7B)" =~ ${args['model']} ]]; then
      printf "\e[31m%s\e[33m%s\e[31m%s\e[33m%s\e[31m\e[0m\n\n" "Invalid option for " "model" ": " "${args['model']}" >&2
      generate-stream_usage >&2
      exit 1
    fi
  fi
  if [[ -n "${args['use-string']}" ]]; then
    if [[ ! "(true false)" =~ ${args['use-string']} ]]; then
      printf "\e[31m%s\e[33m%s\e[31m%s\e[33m%s\e[31m\e[0m\n\n" "Invalid option for " "use-string" ": " "${args['use-string']}" >&2
      generate-stream_usage >&2
      exit 1
    fi
  fi
