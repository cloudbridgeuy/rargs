...
use crate::rargc_value::RargcValue;
use crate::utils::{
...
pub trait Param {
    ...
    fn get_arg_value(&self, matches: &ArgMatches) -> Option<RargcValue>;
...
impl Param for FlagParam {
    ...
    fn get_arg_value(&self, matches: &ArgMatches) -> Option<RargcValue> {
        if matches.get_flag(&self.name) {
            Some(RargcValue::Single(self.name.clone(), "1".to_string()))
        } else {
            None
        }
    }
    ...
    fn get_arg_value(&self, matches: &ArgMatches) -> Option<RargcValue> {
        ...
        if self.multiple {
            let values = matches
                ...
                .unwrap_or_default();
            Some(RargcValue::Multiple(self.name.clone(), values))
        } else {
            let value = escape_shell_words(matches.get_one::<String>(&self.name).unwrap());
            Some(RargcValue::Single(self.name.clone(), value))
        }
    }
    ...
    fn get_arg_value(&self, matches: &ArgMatches) -> Option<RargcValue> {
        ...
        if self.multiple {
            let values = matches
                ...
                .map(|vals| vals.map(|v| escape_shell_words(v)).collect::<Vec<_>>())
                .unwrap_or_default();
            Some(RargcValue::PositionalMultiple(self.name.clone(), values))
        } else {
            let value = escape_shell_words(matches.get_one::<String>(&self.name).unwrap());
            Some(RargcValue::PositionalSingle(self.name.clone(), value))
        }
    }
